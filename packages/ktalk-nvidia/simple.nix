...
pkgs.stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    # Create a wrapper script that uses appimage-run
    cat > $out/bin/ktalk-nvidia << 'WRAPPER_EOF'
#!/usr/bin/env bash
...
WRAPPER_EOF
    ...
    ${pkgs.appimageTools}/bin/appimage-extract "${src}" -o /tmp/ktalk-extract 2>/dev/null || true
    
    # Find and copy icon
    find /tmp/ktalk-extract -name "*.png" -o -name "*.svg" | head -5 | while read icon; do
      icon_name=$(basename "$icon")
      icon_dir=$(dirname "$icon")
      icon_size=$(echo "$icon_dir" | grep -oE "[0-9]+x[0-9]+" || echo "scalable")
