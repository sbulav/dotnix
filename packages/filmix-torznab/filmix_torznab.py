...
def results_xml(items: list[dict], host: str) -> bytes:
    ...
    for it in items:
        ...
        for name, value in [
            ...
            # filmix exposes no swarm stats; report 1/1 so the *arrs
            # don't reject the release as dead.
            ("seeders", "1"),
            ("peers", "1"),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def grab(self, path: str) -> None:
        ...
        if not m:
            ...
            return
        r = SESSION.get(f"{CFG.base_url}/download-file/{m.group(1)}", timeout=60)
        r.raise_for_status()
        if not r.content.startswith(b"d"):  # not bencode: login page or error
            ...
            return
        self.send_body(200, "application/x-bittorrent", r.content)
