...
in
{
  options.${namespace}.containers.nfs = with lib.types; {
    ...
    filesystems =
      mkOpt (listOf str) [ ]
        "List of filesystems to export, in the format ['/path/to/dir']";
    restrictedClients = mkOpt (listOf str) [
      "192.168.80.0/20"
    ] "List of allowed client IPs for NFS exports";
  };
    ...
    services.nfs.server.exports = lib.concatStringsSep "\n" (
      map (
        ...
        let
          ...
          clients = if isIpcam then [ "*" ] else cfg.restrictedClients;
          extraOpts =
            if isIpcam then "all_squash,anonuid=${toString anonUid},anongid=${toString anonGid}" else "";
        in
        lib.concatStringsSep "\n" (
          map (
            ...
            "${fs} ${client}(rw,sync,no_subtree_check,insecure${
              if extraOpts != "" then "," + extraOpts else ""
            })"
          ) clients
        )
      ) cfg.filesystems
