  ...
  config = mkIf cfg.enable {
    programs.wezterm = {
      ...
      extraConfig =
        ...
        ''
        + ''
          -- wmux-managed domains (~/.config/wezterm/wmux-domains.lua is
          -- generated by the wmux home-manager module); absent file is fine
          local wmux_ok, wmux_domains = pcall(dofile, wezterm.config_dir .. "/wmux-domains.lua")
          if wmux_ok and type(wmux_domains) == "table" then
            config.unix_domains = config.unix_domains or {}
            for _, d in ipairs(wmux_domains) do
              table.insert(config.unix_domains, d)
            end
          end
        ''
